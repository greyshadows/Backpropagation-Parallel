...
for j = 1:epochs                %backpropagation algorithm to modify the weights
	...
	%disp(j);
	l1 = act_sigmoid(matrixmultparallel(input_V,W0),0);
	l2 = act_sigmoid(matrixmultparallel(l1,W1),0);
	l2_delta = (output_V - l2) .* act_sigmoid(l2,1);
	l1_delta = (matrixmultparallel(l2_delta,W1')) .* act_sigmoid(l1,1);
	W1 = W1 + (matrixmultparallel(l1',l2_delta)*eeta);
	W0 = W0 + (matrixmultparallel(input_V',l1_delta)*eeta);
    pp=toc;
    time2 = time2 + pp;
    %delete(gcp);
end
